...
function commencerLeQuiz() {
    ...
    historique.push({date: new Date().toLocaleDateString("fr-CA"),responses:[]});
...
function verifierReponse(event) {
    ...
    // historique.
    historique[historique.length-1].responses.push(event.target.indexChoix);
...
function afficherFinQuiz() {
    ...
    for (let partie of historique) {
        ...
        zoneFinQuiz.innerHTML += `Reponses :  ${partie.responses}/ <br>`;
    }
}
